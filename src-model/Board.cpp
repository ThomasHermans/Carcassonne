...
#if 0
void
Board::checkForOccupiedRoads( unsigned inCol, unsigned inRow )
{
	if ( !getTile( inCol, inRow ) )
	{
		return;
	}
	std::vector< Tile::ContiguousRoad > toBeUpdatedRoads = getTile( inCol, inRow )->getContiguousRoads();
	for ( unsigned road = 0; road < toBeUpdatedRoads.size(); ++road )
	{
		// Start a tempQueue and add all RoadAreas from this ContRoad to it
		std::vector< LocatedRoad > tempQueue;
		for ( unsigned i = 0; i < toBeUpdatedRoads[road].size(); ++i )
		{
			tempQueue.push_back
			(
		...
		while ( i < tempQueue.size() )
		{
			...
			LocatedRoad currentRoad = tempQueue[i];
			occupied |= mBoard[currentRoad.first]->hasPiece( Area::Area( currentRoad.second ) );
			unsigned neighborLocation = getNeighborLocation( currentRoad );
			if ( neighborLocation < mNrCols * mNrRows && mBoard[neighborLocation] )
			{
				FRCArea::RoadArea neighborSide = oppositeSide( currentRoad.second );
				// If neighbor is occupied by a piece, set occupied true
				if ( mBoard[neighborLocation]->hasPiece( Area::Area( neighborSide ) ) )
				{
					occupied = true;
				}
				// If neighbor is not yet in tempQueue
				if ( std::find(tempQueue.begin(), tempQueue.end(), LocatedRoad( neighborLocation, neighborSide )) == tempQueue.end() )
				{
					Tile::ContiguousRoad contRoad = mBoard[neighborLocation]->getContiguousRoad(neighborSide);
					for ( unsigned j = 0; j < contRoad.size(); ++j )
...
void
Board::checkForOccupiedCities(unsigned inCol, unsigned inRow)
{
	if ( !getTile( inCol, inRow ) )
	{
		return;
	}
	std::vector< Tile::ContiguousCity > toBeUpdatedCities = getTile( inCol, inRow )->getContiguousCities();
	for ( unsigned city = 0; city < toBeUpdatedCities.size(); ++city )
	{
		// Start a tempQueue and add all CityAreas from this ContCity to it
		std::vector< LocatedCity > tempQueue;
		for ( unsigned i = 0; i < toBeUpdatedCities[city].size(); ++i )
		{
			tempQueue.push_back
			(
...
void
Board::checkForOccupiedFields(unsigned inCol, unsigned inRow)
{
	if ( !getTile( inCol, inRow ) )
	{
		return;
	}
	std::vector< Tile::ContiguousField > toBeUpdatedFields = getTile( inCol, inRow )->getContiguousFields();
	for ( unsigned field = 0; field < toBeUpdatedFields.size(); ++field )
	{
		// Start a tempQueue and add all FieldAreas from this ContField to it
		std::vector< LocatedField > tempQueue;
		for ( unsigned i = 0; i < toBeUpdatedFields[field].size(); ++i )
		{
			tempQueue.push_back
			(
		...
		while ( i < tempQueue.size() )
		{
			...
			unsigned neighborLocation = getNeighborLocation( currentField );
			if ( neighborLocation < mNrCols * mNrRows && mBoard[neighborLocation] )
			{
...
void
Board::checkForOccupiedCloister( unsigned inCol, unsigned inRow )
{
	if ( !isTile( inCol, inRow ) )
	{
		return;
	}
	if ( getTile( inCol, inRow )->getCenter() == Tile::Cloister && getTile( inCol, inRow )->hasPiece( Area::Central ) )
	{
		getTile( inCol, inRow )->occupyCloister();
	}
}
#endif
